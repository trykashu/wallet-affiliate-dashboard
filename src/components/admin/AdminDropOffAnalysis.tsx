...
function severityColor(count: number, stageTotal: number): string {
  if (stageTotal === 0) return "ad-text-3";
  const rate = count / stageTotal;
  if (rate >= 0.4) return "text-[var(--ad-neg)]";
  if (rate >= 0.2) return "text-amber-500";
  return "text-yellow-500";
}

export default function AdminDropOffAnalysis({ users, events }: Props) {
  ...
  if (users.length === 0) {
    return (
      <div className="ad-card p-8 text-center">
        <h3 className="text-sm font-semibold ad-text-1 mb-1">Drop-Off Analysis</h3>
        <p className="text-xs ad-text-3">No data available yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="ad-card p-4 sm:p-6">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h3 className="text-sm font-semibold ad-text-1">Drop-Off Analysis</h3>
          <p className="text-xs ad-text-3 mt-0.5">Where users stop progressing</p>
        </div>
        <div className="text-right">
          <p className="text-xl font-bold text-[var(--ad-neg)]">{overallDropRate}%</p>
          <p className="text-[10px] ad-text-3">overall drop rate</p>
        </div>
      </div>
          ...
          return (
            <div key={slug} className="group">
              ...
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium ad-text-2">{funnelLabel(slug)}</span>
                <div className="flex items-center gap-2">
                  <span className={`text-xs font-semibold ${severity}`}>{count} stopped</span>
                  {stageDropRate > 0 && (
                    <span className="text-[10px] ad-text-3">({stageDropRate}% of total)</span>
                  )}
                </div>
              </div>

              {/* Progress bar */}
              <div className="h-2 bg-[var(--ad-inset)] rounded-full overflow-hidden mb-2">
                <div
                  className="h-full rounded-full bg-[rgba(242,112,110,0.7)] transition-all duration-500"
              ...
              {insight && count > 0 && (
                <div className="mt-2 rounded-lg border border-[var(--ad-border)] bg-[var(--ad-inset)] p-3">
                  <div className="flex items-start gap-2 mb-2">
                    <div className="w-4 h-4 rounded bg-[rgba(244,193,82,0.15)] flex items-center justify-center flex-shrink-0 mt-0.5">
                      <svg className="w-2.5 h-2.5 text-[#F4C152]" aria-hidden="true" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 18v-5.25m0 0a6.01 6.01 0 001.5-.189m-1.5.189a6.01 6.01 0 01-1.5-.189m3.75 7.478a12.06 12.06 0 01-4.5 0m3.75 2.383a14.406 14.406 0 01-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 10-7.517 0c.85.493 1.509 1.333 1.509 2.316V18" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold ad-text-3 uppercase tracking-wider mb-0.5">Why this happens</p>
                      <p className="text-xs ad-text-2 leading-relaxed">{insight.hypothesis}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-2">
                    <div className="w-4 h-4 rounded bg-[rgba(52,211,153,0.15)] flex items-center justify-center flex-shrink-0 mt-0.5">
                      <svg className="w-2.5 h-2.5 text-[var(--ad-pos)]" aria-hidden="true" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold ad-text-3 uppercase tracking-wider mb-0.5">Recommended action</p>
                      <p className="text-xs ad-text-2 leading-relaxed">{insight.action}</p>
                    </div>
                  </div>
                </div>
              )}
      ...
      {/* Summary */}
      {sorted.length > 0 && (
        <div className="mt-5 pt-4 border-t border-[var(--ad-border)]">
          <div className="grid grid-cols-3 gap-3">
            <div className="text-center">
              <p className="text-lg font-bold ad-text-1">{totalDropped}</p>
              <p className="text-[10px] ad-text-3">Total Stopped</p>
            </div>
            <div className="text-center">
              <p className="text-lg font-bold text-[var(--ad-neg)]">{overallDropRate}%</p>
              <p className="text-[10px] ad-text-3">Drop Rate</p>
            </div>
            <div className="text-center">
              <p className="text-lg font-bold ad-text-1">
                {sorted[0] ? funnelLabel(sorted[0].slug) : "\u2014"}
              </p>
              <p className="text-[10px] ad-text-3">Biggest Bottleneck</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 mt-3">
            <div className="flex items-center gap-1.5 text-[10px]">
              ...
              <span className="ad-text-3">&lt;20% drop</span>
            </div>
            <div className="flex items-center gap-1.5 text-[10px]">
              <span className="w-2 h-2 rounded-full bg-amber-500" />
              <span className="ad-text-3">20-40% drop</span>
            </div>
            <div className="flex items-center gap-1.5 text-[10px]">
              <span className="w-2 h-2 rounded-full bg-red-500" />
              <span className="ad-text-3">&gt;40% drop</span>
            </div>
          </div>
        </div>
      )}
      ...
      {sorted.length === 0 && totalUsers > 0 && (
        <div className="text-center py-6">
          <p className="text-sm ad-text-3">All users have completed the full funnel. Great job!</p>
        </div>
      )}
    </div>
  );
