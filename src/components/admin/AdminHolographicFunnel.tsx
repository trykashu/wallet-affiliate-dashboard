...
/* ── Component ─────────────────────────────────────────── */
export default function AdminHolographicFunnel({
  users,
  statuses,
  stageDurations,
  events,
}: Props) {
  // Admin shell has no whitelabel brand context — always Kashu defaults.
  const accentHex: string | undefined = undefined;
  const sidebarBgHex: string | undefined = undefined;
  ...
  const animate = useCallback(() => {
    ...
    // Fade trail — dark bg
    ctx.fillStyle = "rgba(20,24,33,0.42)";
  ...
  if (total === 0) {
    return (
      <div className="ad-card p-8 text-center">
        <h3 className="text-sm font-semibold ad-text-1 mb-1">
          Conversion Funnel
        </h3>
  ...
  return (
    <div className="ad-card overflow-hidden">
      {/* Header */}
      <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-[var(--ad-border)] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-semibold ad-text-1">
              Conversion Funnel
            </h3>
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider bg-[rgba(52,211,153,0.10)] text-[var(--ad-pos)] border border-[rgba(52,211,153,0.28)]">
              <span className="w-1 h-1 rounded-full bg-[var(--ad-pos)] animate-pulse" />
              Live
            </span>
          </div>
          <p className="text-xs ad-text-3 mt-0.5">
            Track user progression &middot;{" "}
            <span className="font-semibold ad-text-2 tabular-nums">
              {fmt.count(funnelTop)}
            </span>{" "}
            users &middot;{" "}
            <span className="font-semibold ad-text-2 tabular-nums">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[11px] font-semibold ad-text-2 uppercase tracking-wider truncate">
                      {stage.label}
                    </span>
                    <span className="text-sm font-bold ad-text-1 tabular-nums flex-shrink-0">
                      {stage.reachedCount.toLocaleString()}
                    </span>
                  </div>

                  <div className="flex items-center gap-2 mt-0.5">
                    <div className="flex-1 h-1 bg-[var(--ad-inset)] rounded-full overflow-hidden">
                      <div
                    ...
                    <span className="text-[10px] font-bold tabular-nums ad-text-3 w-10 text-right">
                      {ofTotal.toFixed(0)}%
                    </span>
                  </div>
      ...
      {/* ── Summary Footer ───────────────────────────────── */}
      <div className="px-4 sm:px-6 py-4 border-t border-[var(--ad-border)] bg-[var(--ad-inset)]">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-[var(--ad-inset)] rounded-xl px-4 py-3 border border-[var(--ad-border)]">
            <p className="text-[10px] uppercase tracking-[0.1em] ad-text-3 font-medium mb-1">
              Overall Conversion
            </p>
            <p className="text-lg sm:text-xl font-bold ad-text-1 tabular-nums">
              {overallConversion.toFixed(1)}%
            </p>
          </div>
          <div className="bg-[var(--ad-inset)] rounded-xl px-4 py-3 border border-[var(--ad-border)]">
            ...
            <p className="text-lg sm:text-xl font-bold text-[var(--ad-neg)] tabular-nums">
