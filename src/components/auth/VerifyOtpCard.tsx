...
/**
 * Click-to-verify card for emailed auth links.
 *
 * Two delivery hazards shape this design:
 * 1. Email security scanners (Gmail prefetch, Microsoft SafeLinks) follow
 *    links with GET requests and burn one-time Supabase tokens before the
 *    user clicks. So loading the page consumes NOTHING — the token is only
 *    verified when the user clicks the button (an XHR from a real gesture;
 *    scanners issue bare GETs and don't submit).
 * 2. The SMTP relay behind our sender mis-decodes quoted-printable: any
 *    literal `=` followed by two hex chars is eaten (corrupting
 *    `?token_hash=<hex>` in every email). So emailed links use the
 *    PATH format /auth/verify/<type>/<token_hash> — zero `=` characters.
 *    Query format (?token_hash=...&type=...) is still accepted for links
 *    we deliver outside that relay.
 *
...
interface VerifyOtpCardProps {
  /** Pre-parsed from the URL path; when absent, query params are read on mount. */
  tokenHash?: string;
  type?: string;
}

export default function VerifyOtpCard({ tokenHash: pathTokenHash, type: pathType }: VerifyOtpCardProps) {
  ...
  useEffect(() => {
    ...
    const tokenHash = pathTokenHash ?? search.get("token_hash");
    const type = (pathType ?? search.get("type")) as EmailOtpType | null;
      ...
      <div className="min-h-screen bg-surface-50 flex items-center justify-center px-4">
        <div className="text-center max-w-sm">
          ...
          <p className="mt-4 text-xs text-brand-400">
            From the login page you can request a fresh sign-in link any time.
          </p>
        </div>
      </div>
    );
